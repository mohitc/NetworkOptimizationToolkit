...
public class ForwardingBasedRoutingConstrNameGenerator extends LPNameGeneratorImpl<String> {
  ...
  private static final String PREFIX = "FORWARDING-ROUTING";
  ...
  protected void validatePrefixConstraint(List<String> strings) throws LPNameException {
    ...
    //c) D destination cannot be start node I of a link
    if (strings.get(1).equals(strings.get(2))) {
      throw new LPNameException("Destination cannot be start node of a link");
    }

    //d) Link indices should be unique (i!=j)
